...
def read_salary_data(csv_file):
    with open(csv_file) as csvfile:
        ...
        for row in reader:
            ...
            row['PPD'] = Decimal(
                row['AvgPointsPerGame']) / Decimal(row['Salary'])
            for p in positions:
                r = copy.copy(row)
                r['Position'] = p
...
def scan_lineups(sorted_salaries, num_lineups, selections):
    ...
    while lineup_count < num_lineups:
        ...
        current_lineup.append(
            sorted_salaries[[position]][index_map[[position]]])
        index_map[worst_player['Position']] = (
            index_map[worst_player['Position']] + 1)
        salary_total = 0
        points_total = 0
        for p in current_lineup:
            salary_total = salary_total + int(p['Salary'])
...
def main():
    ...
    if sport == 'baseball':
        filename = 'data/Baseball20160822.csv'
        selections = {
            'SP': 2,
            'C': 1,
            '1B': 1,
